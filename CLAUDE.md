...
fluttersdk_telescope`) so `dart run fluttersdk_telescope <cmd>` invocations work without dragging `dart:ui` into a
pure-Dart context, and a `lib/cli.dart` codegen barrel (typedef `FluttersdkTelescopeArtisanProvider`) consumed by
consumer-side `lib/app/_plugins.g.dart` auto-discovery. An `install.yaml` manifest at the package root makes
